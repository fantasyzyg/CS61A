...
def find_centroid(cluster):
    ...
    xs, ys = [x[0] for x in locations], [x[1] for x in locations]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum([(x[0]-meanX)*(x[1]-meanY) for x in zip(xs, ys)])
    b = Sxy / Sxx
    a = meanY - b*meanX
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            result[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            result[restaurant_name(restaurant)] = predictor(restaurant)
