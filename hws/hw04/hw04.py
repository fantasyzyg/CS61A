...
def balanced(m):
    ...
    if is_weight(m):
        ...
        left_balanced, right_balanced = balanced(left_end), balanced(right_end)
        current_balanced = length(left(m))*total_weight(left_end) == length(right(m))*total_weight(right_end)
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        w = total_weight(m)
        return tree(w, [totals_tree(end(left(m))), totals_tree(end(right(m)))])
